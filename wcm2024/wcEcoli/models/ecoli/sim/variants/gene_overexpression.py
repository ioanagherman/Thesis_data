...
Expected variant indices (depends on length of sim_data.process.transcription.rna_data):
	0: control
	1-4692: gene index to overexpress
"""
...
def gene_overexpression(sim_data, index):
	...
	factor = 50  # Overexpression
	sim_data.adjust_final_expression([geneIndex], [factor])
	geneID = rna_data["id"][geneIndex]

	return dict(
		shortName = "{}_overexpression".format(geneID),
		desc = "Overexpression of {}.".format(geneID) + "Factor used {}".format(factor)
		), sim_data
