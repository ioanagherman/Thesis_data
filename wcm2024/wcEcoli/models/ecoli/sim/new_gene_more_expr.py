...
NEW_GENE_INDUCTION_GEN = 6 # Generation index to induce new gene expression
...
NEW_GENE_EXPRESSION_FACTORS = [0, 6, 7]
...
NEW_GENE_EXPRESSION_FACTORS1 = np.linspace(10**(NEW_GENE_EXPRESSION_FACTORS[1]-1), 10**(NEW_GENE_EXPRESSION_FACTORS[2]-1), 5)
NEW_GENE_EXPRESSION_FACTORS2 = np.linspace(10**(NEW_GENE_EXPRESSION_FACTORS[1]), 10**(NEW_GENE_EXPRESSION_FACTORS[2]), 5)

NEW_GENE_EXPRESSION_FACTORS = [0] + [int(x) for x in NEW_GENE_EXPRESSION_FACTORS1[1:-1]] + [int(x) for x in NEW_GENE_EXPRESSION_FACTORS2[1:-1]]
print('new_gene_more_expr', NEW_GENE_EXPRESSION_FACTORS)
#OVEREXPR = [
#    (1011, 1012,)
#]
...
def get_new_gene_expression_factor_and_translation_efficiency(sim_data, index):
	...
	Returns:
		expression_factor: this is taken directly from NEW_GENE_EXPRESSION_FACTORS
...
def new_gene_more_expr(sim_data, index):
	"""
	...
	#			  + "{}_env".format(sim_data.ordered_conditions[condition_index]) + overexpr['shortName'],
